...
# ------------------------------------------------------------------
#  EXCEL STYLER  –  RAM-only + secure wipe
# ------------------------------------------------------------------
def style_excel(df: pd.DataFrame) -> io.BytesIO:
    ...
    try:
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            ...
            for r in range(2, len(df) + 2):
                ...
                for c in (2, 4, 5):
                    ...
                    if df.iloc[r - 2, 0] == "Total":
                        cell.font = bold

            for c in range(1, len(df.columns) + 1):
                ws.cell(row=len(df) + 1, column=c).font = bold
        ...
        if tmp and os.path.exists(tmp):
            with open(tmp, "ba+", buffering=0) as f:
                f.write(b"\0" * os.path.getsize(tmp))
            os.remove(tmp)
